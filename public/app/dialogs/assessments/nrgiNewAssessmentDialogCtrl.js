...
angular.module('app')
    .controller('nrgiNewAssessmentDialogCtrl', function (
        $scope,
        $route,
        $location,
        ngDialog,
        ...
        $scope.assessments = [];
        $scope.$parent.assessments.forEach(function (assessment) {
            $scope.assessments.push(assessment.assessment_ID);
        });
        ...
        $scope.assessmentDeploy = function () {
            if ($scope.assessments.indexOf($scope.new_assessment.assessment_country.iso2 + '-' + String($scope.new_assessment.year)) > -1) {
                ...
                nrgiQuestionSrvc.query({assessment_ID: 'base'}, function (questions) {

                    questions.forEach(function (question) {
                        new_answer_data.push({
                            answer_ID: $scope.new_assessment.assessment_country.iso2 + '-' + String($scope.new_assessment.year) + '-' + String(nrgiUtilsSrvc.zeroFill(question.question_flow_order, 3)),
                            ...
                            question_flow_order: question.question_flow_order,
                            question_text: question.question_text,
                            question_secondary_source: question.question_secondary_source,
                            question_mode: question.question_mode,
                            question_data_type: question.question_data_type,
                            question_indicator: question.question_indicator,
                            question_indicator_ID: question.question_indicator_ID,
                            question_theme_ID: question.question_theme_ID,
                            question_value_chain_ID: question.question_value_chain_ID,
